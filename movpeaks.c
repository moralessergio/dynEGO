...
int change_frequency=0; //= CHANGE_FREQ; /* number of evaluations between changes. change_frequency=0 means that function never changes (or only if function change_peaks is called)*/

unsigned long int movrandseed = 1; /* seed for built-in random number generator */

int geno_size = D;  /* number of dimensions, or the number of double 
                          valued genes */
double vlength = 1.0; /* distance by which the peaks are moved, severity */

double height_severity=4.0; /* severity of height changes, larger numbers
                               mean larger severity */
double width_severity = 0.01; /* severity of width changes, larger numbers
                               mean larger severity */
...
int number_of_peaks = 5; /* number of peaks in the landscape */
...
//double minwidth = 0.8, maxwidth = 7.0, standardwidth = 0.0;
//double minwidth = 0.0001, maxwidth = 1.0, standardwidth = 0.1;
double minwidth = 0.05, maxwidth = 0.15, standardwidth = 0.1;
//double minwidth = 1.5, maxwidth = 2.5, standardwidth = 2;        
...
void init_peaks (unsigned long int seed, double vLen, double hSev, double wSev, int inPeakNum)
{

	/* Sets the initial random seed and other parameters (SME) */
	movrandseed = seed;
	vlength = vLen;
	height_severity = hSev;
	width_severity = wSev;
	number_of_peaks = inPeakNum;
...
double eval_movpeaks (double *gen)
{
  ...
  if ((change_frequency > 0)&&(evals%change_frequency==0)&&evals>0)	//SME - 20120301
    change_peaks();
...
/*Returns a random number from a modified uniform distribution so that peaks are more likely to happen in the edges than in the centre. 
This is to test the theory that it might be the case that using a sampling strategy with memory (from previous epochs) is helping even
when drastic changes happen. The idea comes from beinge twice as likely to hit a good sample if the peak is in the centre (up hill and down hill
than when the peak is close to the edges (onlu uphill is available => less likely to have a good hit)*/
double getBiasedRandMove(){
	double u = movrand();
	int iCount = 0;
	//while(iCount<100){
	while(1){
		if( fabs(u- 0.5) >= movrand() )//accept		
			break;
		else			      //redraw a move
			u = movrand();
		iCount++;
...
/* whenever this function is called, the peaks are completely (and randomly) changed (reinitialized) */ 
void drasticChange_peaks()
{
  int i,j;
  double dummy;
  for (i=0; i< number_of_peaks; i++){
//    printf("Peak %i coords: \t",i);
    for (j=0; j< geno_size; j++){
  	  //peak[i][j] = (maxcoordinate - mincoordinate) * movrand() + mincoordinate;
	    peak[i][j] = (maxcoordinate - mincoordinate) * getBiasedRandMove() + mincoordinate;
      	  prev_movement[i][j] = movrand()-0.5;
//	  printf("%f\t",peak[i][j]);
    }
    peak[i][geno_size+1]= (maxheight-minheight)*movrand()+minheight;
    peak[i][geno_size]= (maxwidth-minwidth)*movrand()+minwidth;
//    printf("height: %f\t width: %f\n",peak[i][geno_size+1],peak[i][geno_size]);
  }
...
double get_global_max() /* returns the average error of all evaluation calls so far */
{
  return global_max;
}
...
int get_maximum_peak()
{
	return maximum_peak;
}
double get_w(int inPeak){
	return 	peak[inPeak][D];
}
